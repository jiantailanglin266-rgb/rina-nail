...
## 予約の設定はサイト側とGAS側の両方を同じ値にする

営業時間・メニュー・料金・準備時間・受付ルールは、次の2か所に同じ値で存在します。

| | サイト側 | サーバー側（Google Apps Script） |
|---|---|---|
| 営業時間 | `src/data/hours.ts` | `CONFIG.businessHours` |
| メニュー | `src/data/booking/menus.ts` | `MENUS` / `OPTIONS` |
| 受付ルール | `src/data/booking/settings.ts` | `CONFIG` |

**片方だけ変えると「画面では選べるのに確定できない」状態になります。**
サーバー側が拒否する（安全側に倒れる）ため事故にはなりませんが、
お客様には不可解な失敗として見えます。必ず両方を更新してください。

## 予約の確定判断はサーバー側だけが行う

`src/lib/booking/availability.ts` の計算はブラウザ側でも動きますが、
これは「選べる枠を見せる」ためのものです。
画面を開いてから確定するまでの間に他の方が予約する可能性があるため、
**重複の最終判定は必ず Google Apps Script 側**（`assertBookable` と `LockService`）で行います。
ブラウザ側の検証だけに頼る実装を追加しないでください。

## メール送信の失敗で予約を取り消さない

カレンダー登録が成功した時点で予約は成立しています。
メール送信の失敗で例外を投げると、お客様が再送信して二重予約になります。
GAS側では送信失敗を記録するだけにとどめています（`markMailFailed`）。
